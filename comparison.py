...
Purpose: Study population description and group comparison
...
# Create function to convert list into dataframe
def ls_to_df(ls):
    new_df = []
    for df in ls :
...
ordinal_var = ['ord_x','ord_y']
continuous_var = ['cont_x', 'cont_y']

# Calculate missing data and percentages
#count of missing data for each variable
null = []
for x in [ordinal_var, continuous_var]: 
    null += [data[x].isna().sum()]
ordinal_na, continuous_na = null

#percentage of missing data for each variable
per_null = []
for x in [ordinal_na, continuous_na]: 
    per_null += [(x/len(data.index))*100]
ordinal_perna, continuous_perna = per_null

# Create dataframe of continuous data

  # Missing data : count
df_count = pd.DataFrame(continuous_na)
df_count.columns = ['missing_count']
  # Missing data : percentages
df_per = pd.DataFrame(continuous_perna)
df_per.columns = ['missing_percentage']
  # Merging count and percentages using index
df_continuous_missing = pd.merge(df_count, df_per, left_index=True, right_index=True)

# Create a function to calculate descriptive statistics (groups = 0, 1)
def desc_continuous(df, groups):
    #Print n of groups
    n = []
    n += [{'n{}'.format(status): len(df[df[group] == status]) for status in groups}]
    ...
    for status in groups:
        ...
        for columns in [continuous_var]:
            result1 += [df[columns][df[group] == status].mean(axis=0)]
            result2 += [df[columns][df[group] == status].sem(axis=0)]
        mean += [result1]
        sem += [result2]
